package com.example.seerbitsdk.component

object Route{

    const val CARD_OTP_SCREEN: String = "CARD_OTP_SCREE"
    const val MOMO_OTP_SCREEN: String = "momootpscreen"
    const val BANK_ACCOUNT_REDIRECT_URL_SCREEN: String = "BankAccountRedirectUrlScreen"
    const val OnBoardingScreen = "OnBoardingScreen"
    const val PIN_SCREEN = "PinScreen"
    const val OTHER_PAYMENT_SCREEN = "OtherPaymentScreen"
    const val USSD_HOME_SCREEN = "USSDHomeScreen"
    const val CARD_ACCOUNT_REDIRECT_URL_SCREEN: String = "CARDRedirectUrlScreen"

    const val BANK_ACCOUNT_NUMBER_SCREEN = "BankAccountNumberScreen"
    const val BANK_ACCOUNT_NAME_SCREEN = "BankAccountNamerScreen"
    const val BANK_ACCOUNT_BVN_SCREEN = "BankAccountBvnScreen"
    const val BANK_ACCOUNT_DOB_SCREEN = "BankAccountDobScreen"
    const val BANK_ACCOUNT_OTP_SCREEN = "BankAccountOtpScreen"


}